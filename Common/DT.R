...
library(rpart.plot)
library(pROC)
training<-read.csv("F:/Thesis/DataMing+MachieLeaning/Diabetes/Common/Normalized_Dataset_Train80p_Com6.csv")
testing<-read.csv("F:/Thesis/DataMing+MachieLeaning/Diabetes/Common/Normalized_Dataset_Test20p_Com6.csv")
sapdata<-read.csv("F:/Thesis/DataMing+MachieLeaning/Diabetes/Common/Sap_DT_Selected.csv")
...
for(cp_value in 1:length(cp_array)){
  for (i in 1:20) {
    for (j in 1:32) {
      ...
      testing$pred <- predict(object = prepruned_model,  newdata = testing[,-7],    type = "class")
      #printcp()
      #plotcp()
      ...
      roc_obj<-roc(testing[,7],as.numeric(testing$pred))
      rocauc<-auc(roc_obj)
...
write.csv(sapdata,"F:/Thesis/DataMing+MachieLeaning/Diabetes/Common/Sap_DT_Selected.csv")
print(cp_op)
print(max_dep)
print(min_split)
print(accu)
