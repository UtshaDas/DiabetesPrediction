library(neuralnet)
library(pROC)
trainDF<-read.csv("F:/Thesis/DataMing+MachieLeaning/Diabetes/Common/Normalized_Dataset_Train80p_Com6.csv")
testDF<-read.csv("F:/Thesis/DataMing+MachieLeaning/Diabetes/Common/Normalized_Dataset_Test20p_Com6.csv")
...
neuralModel<-neuralnet(formula=form, data=trainDF, hidden = c(4,2), threshold = 0.001,
...
PredictionsWithClass <- compute(neuralModel,testDF[,1:6]) 
