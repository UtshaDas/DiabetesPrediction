rm(list=ls())
training<-read.csv("F:/Thesis/DataMing+MachieLeaning/Diabetes/mRMR/Normalized_Dataset_Train80p_top10.csv")
testing<-read.csv("F:/Thesis/DataMing+MachieLeaning/Diabetes/mRMR/Normalized_Dataset_Test20p_top10.csv")
sapdata<-read.csv("F:/Thesis/DataMing+MachieLeaning/Diabetes/mRMR/Sap_SVM_Selected.csv")
...
for(i in 1:length(seedarray)){
  ...
  system.time(svm_tune <- tune(svm, train.x=training[,1:10], train.y=as.factor(training[,11]), 
  ...
  print(classifier)
  y_pred=predict(classifier,newdata=testing[-11])
  #y_pred<-round(y_pred)
  cm=table(y_pred,testing[,11],dnn=c("Prediction","Actual"))
  ...
  roc_obj<-roc(testing[,11],as.numeric(y_pred))
  rocauc<-auc(roc_obj)
...
write.csv(sapdata,"F:/Thesis/DataMing+MachieLeaning/Diabetes/mRMR/Sap_SVM_Selected.csv")
